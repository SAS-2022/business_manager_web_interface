...
This directory is a complete, self-contained static site: `index.html`,
`robots.txt`, `sitemap.xml`, `assets/`. Point any static host at it as-is —
no build command needed. Firebase Hosting can serve it as a second "site"
target in the same project alongside the Flutter app's own Hosting target;
Netlify/Vercel/S3+CloudFront/GitHub Pages all work too by just uploading
this folder.

## Known placeholders — fix before going live

- **Every Login / Get Started / Get Premium / Contact Us link points at
  `app.costera.biz`**, which doesn't exist yet — no DNS/hosting is set up
  for the app subdomain. Once that's live, update these (a find-and-replace
  across `index.html` is enough; there's no build step to worry about).
  Note this is a *separate* piece of hosting from this static site: `costera.biz`
  (this folder) and `app.costera.biz` (the Flutter app) are two different
  domains/hosting targets, each needing its own DNS record — deploying this
  folder to `costera.biz` does not make `app.costera.biz` exist.
...
## The share image (`assets/social-preview.png`)
