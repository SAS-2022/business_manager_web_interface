...
// ── Design constants ──────────────────────────────────────────────────────────
// Purple accent for quotations
const double _kMargin = 40.0;
const double _kLogoSize = 56.0;
const double _kHeaderBgH = 110.0;
// Purple: PdfColor(106, 27, 154)
// Light purple accent text: PdfColor(74, 20, 140)
// Purple bg strip: PdfColor(249, 245, 255)

Future<PdfDocument> generateQuotation({
  ...
  for (int i = 0; i < order.orderedProducts!.length; i += 6) {
    ...
    // ── Header: purple left accent bar + light bg ─────────────────────────
    graphics.drawRectangle(
      bounds: const Rect.fromLTWH(0, 0, 6, _kHeaderBgH),
      brush: PdfSolidBrush(PdfColor(106, 27, 154)),
    );
    graphics.drawRectangle(
      bounds: Rect.fromLTWH(6, 0, pageWidth - 6, _kHeaderBgH),
      brush: PdfSolidBrush(PdfColor(250, 245, 255)),
    );
    ...
    graphics.drawString(
      ...
      brush: PdfSolidBrush(PdfColor(74, 20, 140)),
    );
    ...
    // ── Quotation badge ───────────────────────────────────────────────────
    final String quoteLabel = '${appLoc.quotation}  #${order.uid ?? ''}';
    graphics.drawString(
      quoteLabel,
      fonts['invoiceBadge']!,
      bounds: Rect.fromLTWH(companyX, 74, companyW, 18),
      format: ArabicTextHelper.getFormatForText(quoteLabel),
      brush: PdfSolidBrush(PdfColor(106, 27, 154)),
    );
    ...
    graphics.drawString(
      order.clientName ?? '',
      regularFont,
      bounds: Rect.fromLTWH(col2X, clientY, colW, 18),
    ...
    // Rule before product table — purple
    graphics.drawLine(
      PdfPen(PdfColor(106, 27, 154), width: 1.0),
...
Future<Map<String, dynamic>> _productGrid(
  ...
  headerRow.style = PdfGridRowStyle(
    backgroundBrush: PdfSolidBrush(PdfColor(106, 27, 154)),
    ...
    for (int i = rowAt; i < pageRows; i++) {
      ...
      row.style = PdfGridRowStyle(
        ...
        backgroundBrush: i.isEven
            ? PdfBrushes.white
            : PdfSolidBrush(PdfColor(248, 245, 255)),
...
Future<double> _totalSection(
  ...
  final int totalPages = (order.orderedProducts!.length / 6).ceil();
...
Future<double> _drawTermsAndConditions(
  ...
  if (invoiceSettings.paymentTerms != null ||
    ...
    graphics.drawString(appLoc.paymentTerms, subHeaderFont,
  ...
  if (invoiceSettings.deliveryTerms != null ||
    ...
    graphics.drawString('${appLoc.termsandConditions}:', subHeaderFont,
        bounds: Rect.fromLTWH(termsX, yPos, pageWidth * 0.5, 100),
...
Future<double> _schedueledDelivery(
  ...
  if (order.scheduledDate != null || order.scheduledAt != null) {
    graphics.drawString(appLoc.scheduledOrder, subHeaderFont,
